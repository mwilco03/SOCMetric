# SOC Productivity Dashboard — Coverage, Scheduling & Import Design

**Version:** 1.0  
**Status:** Pre-Implementation  
**Companion to:** Design Document v1.1, UX & Workflow Design v1.0
...
2. [Shift Configuration Model](#2-shift-configuration-model)
3. [Coverage Ledger Model](#3-coverage-ledger-model)
4. [Overlap and Coverage Validation](#4-overlap-and-coverage-validation)
5. [Bulk Import Format](#5-bulk-import-format)
6. [Import Parser Specification](#6-import-parser-specification)
7. [Import Preview UI](#7-import-preview-ui)
8. [Holiday Calendar Integration](#8-holiday-calendar-integration)
9. [After-Hours and Weekend Work Detection](#9-after-hours-and-weekend-work-detection)
10. [Analyst Identity Model](#10-analyst-identity-model)
11. [New Hire Ramp Model](#11-new-hire-ramp-model)
12. [Search and Matching Technology Decisions](#12-search-and-matching-technology-decisions)
13. [Vault Schema Additions](#13-vault-schema-additions)
...
## 4. Overlap and Coverage Validation
...
## 8. Holiday Calendar Integration
...
## 9. After-Hours and Weekend Work Detection
...
## 12. Search and Matching Technology Decisions

### What Was Evaluated

Meilisearch and Quickwit were evaluated. Both are server-side systems. This application has no backend by design. Neither is appropriate.

### Technology Decisions

**Analyst name fuzzy matching (bulk import)**
```
Problem: "Parkr A" should match a known name
Scale:   < 20 names
Tool:    Levenshtein distance, implemented inline, no library
         If distance <= 2: high confidence match, auto-apply with INFO flag
         If distance 3-4:  low confidence, require confirmation in preview
         If distance > 4:  no match, flag as UNRESOLVED
```

**Ticket filtering by cluster / label / source system**
```
Problem: filter thousands of tickets by dimension values
Scale:   5,000-50,000 tickets (realistic SOC volume)
Tool:    Array.filter() on in-memory dataset
         No search engine needed at any realistic SOC ticket volume
         Dimension indices (Map<dimensionValue, ticketKey[]>) built once on load
```

**Keyword clustering / token matching on normalized titles**
```
Problem: group tickets by alert rule name after entity normalization
Scale:   thousands of normalized title strings
Tool:    Map<token, ticketKey[]> built in a web worker on data load
         Token frequency map is built once, queried in O(1)
```

**Recurrence detection (similarity scoring)**
```
Problem: find similar ticket titles within rolling time windows
Scale:   O(n²) per window -- ~10,000 tickets = ~50M comparisons per 30d window
Tool:    Jaccard coefficient on token sets, computed in a dedicated web worker
         Web worker runs off main thread: no UI blocking
         If computation exceeds 2 seconds: switch to MinHash approximation
         MinHash reduces to O(n × k) where k = number of hash functions (typically 128)
         Both are client-side, zero server dependency
```

**Full-text search in ticket list UI**
```
Problem: user types in search box, tickets filter in real time
Scale:   current visible ticket list (hundreds, not thousands)
Tool:    Fuse.js -- 24kb, zero dependencies, client-side fuzzy search
         Configured on the normalized title field
         Threshold: 0.4 (balanced precision/recall for alert-style titles)
```

**Shift name resolution in bulk import**
```
Problem: "day" should match "Day EST" in the shift config
Scale:   < 10 configured shifts
Tool:    Substring match + exact match, inline, no library
```
...
| Tool | Reason |
|---|---|
| Meilisearch | Server-side only, requires backend process |
| Quickwit | Log-scale distributed search, server-side, wrong problem size |
| Elasticsearch | Same category as Quickwit, requires infrastructure |
| Lunr.js | Older library, does not handle entity-normalized SIEM titles well |
| FlexSearch | Viable alternative to Fuse.js but heavier for the use case |

---

## 13. Vault Schema Additions

New fields added to the vault schema defined in Design Document v1.1 Section 16.
...
```json
{
  ...
  "afterHoursConfig": {
    ...
    "irCorrelationLookbackMinutes": 240
  },
  ...
  "newHireRampDefaults": {
    ...
    "rampDays": 90,
    "rampProfile": [
      { "dayRange": [1, 30],  "capacityFraction": 0.25 },
      { "dayRange": [31, 60], "capacityFraction": 0.50 },
      { "dayRange": [61, 90], "capacityFraction": 0.75 }
    ]
  },
  ...
  "contextLedger": [
    {
      "id": "string",
      "type": "absence | new-hire | system | rule | holiday",
      "shift": "string",
      "start": "YYYY-MM-DD",
      "end": "YYYY-MM-DD | null",
      "delta": "number | null"
...
*End of Coverage, Scheduling & Import Design Document v1.0*
